...
```{r include=FALSE}
load("../R_results_controlssgRNAs/result.Rdata")
```
...
df_reduced_info <- unique(subset(DESeq_result_table, DESeq_result_table$time==8 | DESeq_result_table$time==14)[,c(2,30,31,32,4,20,21,23,24,26,27,28,29)])
write_tsv(df_reduced_info, file="../R_results_controlssgRNAs/Reduced_annotated_DESeq_result_table_comparisonsConditions.tsv")
```

```{r wider-table}
df_red_wide <- pivot_wider(df_reduced_info, names_from=condition, values_from=c(wmean_fitness, sd_fitness, p_fitness_adj, comb_score, p_fitness_condition, p_fitness_condition_adj, p_fitness_condition_2, p_fitness_condition_2_adj))
